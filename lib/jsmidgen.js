...
(function(exported) {
	...
	var Util = {
		...
		translateTickTime: function(ticks) {
			let vlqList = [];
			do {
				vlqList.push((ticks & 0x7F) | (0x80 * (ticks > 0x7F)));
				ticks >>= 7;
			} while (ticks);
			return vlqList;
		},
	...
	File.prototype.toBytes = function() {
		...
		bytes = Array.from(encoder.encode(bytes));

		// add the number of ticks per beat (currently hardcoded)
		bytes = bytes.concat(Util.translateTickTime(this.ticks));

		// iterate over the tracks, converting to bytes too
		this.tracks.forEach(function(track) {
			bytes = bytes.concat(track.toBytes());
		});
